...
CASES = [
  dict(slug="lucent", emoji="🔦", title="Lucent",
    ...
    differently=p(C("e.g. lock the GTM motion earlier; investors flagged it as the biggest gap")),
  ...
  dict(slug="ey", emoji="☁️", title="EY, AI & Data",
    ...
    results=results(("$14M", "contract extension secured"), ("−30%", "executive escalations"), ("90%", "adoption across 4,000 users"), ("$10M+", "savings identified over 3+ years")) + "\n" + p("Earlier at EY: logistics lead for a Fortune 500 consumer foods client (weekly KPI reporting and root-cause sessions; on-time delivery +20%, accuracy +10% YoY), price-volume-mix analysis for a Fortune 500 software client's FP&A team (recovered $5M in pricing and mix leakage), and re-architected Agile workflows for 25+ engineering teams (delivery performance +40% YoY). " + C("the doc also mentions an internal AI adoption campaign with 200%+ usage across 5 user segments and an internal data architect role; neither is on the resume, so they are left out until confirmed")),
    differently=p(C("what you'd do differently")),
